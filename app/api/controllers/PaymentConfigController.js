...
class PaymentConfigController {
    ...
    async index(req, res, next) {
        try {
            ...
            const {docs, pages, total} = await models.PaymentConfigs.paginate(options).then(data => {
                for (var i = 0; i < data.docs.length; i++) {
                    data.docs[i].data_config = JSON.parse(data.docs[i].data_config);
                }
                return data;
            });
    ...
    async show(req, res, next) {
        try {
            ...
            const docs = await models.PaymentConfigs.findByPk(req.params.id).then(
                data => {
                    if (data === null) {
    ...
    async create(req, res, next) {
        try {
            ...
            const docs = await models.PaymentConfigs.create(req.body);
            let response = {
                data: docs
            }
        ...
        try {
            ...
            const docs = await models.PaymentConfigs.findByPk(req.params.id).then(
                data => {
                    ...
                    return models.PaymentConfigs.update(req.body, {where: {id: req.params.id}}).then(
                        _data => {
                            if (_data) {
                                return models.PaymentConfigs.findByPk(req.params.id);
                            }
        ...
        try {
            ...
            const docs = await models.PaymentConfigs.findByPk(req.params.id).then(
                data => {
                    ...
                    return models.PaymentConfigs.destroy({where: {id: req.params.id}}).then(
                        _data => {
                            if (_data) {
...
export default new PaymentConfigController();